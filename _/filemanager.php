...
$default_timezone = 'Asia/Makassar'; // UTC
...
if (isset($_GET['view'])) {
    ...
    $file_path = $path . '/' . $file;
    $_file_url = preg_replace('/:[0-9]+/', '', $file_url);
    ...
    <div class="row">
        <div class="col-12">
            ...
            <p>
                ...
                <b><a href="<?php echo fm_enc($_file_url) ?>" target="_blank"><i class="fa fa-external-link-square"></i> Open</a></b>
                &nbsp;
                <?php
                // ZIP actions
            ...
            if($is_onlineViewer) {
                // Google docs viewer
                echo '<iframe src="https://docs.google.com/viewer?embedded=true&hl=en&url=' . fm_enc($_file_url) . '" frameborder="no" style="width:100%;min-height:460px"></iframe>';
            } elseif ($is_zip) {
                // ZIP content
                if ($filenames !== false) {
                ...
                if (in_array($ext, array('gif', 'jpg', 'jpeg', 'png', 'bmp', 'ico'))) {
                    echo '<p><img src="' . fm_enc($_file_url) . '" alt="" class="preview-img"></p>';
                }
            } elseif ($is_audio) {
                // Audio content
                echo '<p><audio src="' . fm_enc($_file_url) . '" controls preload="metadata"></audio></p>';
            } elseif ($is_video) {
                // Video content
                echo '<div class="preview-video"><video src="' . fm_enc($_file_url) . '" width="640" height="360" controls preload="metadata"></video></div>';
            } elseif ($is_text) {
