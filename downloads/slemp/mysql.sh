...
EOF
	...
	if [ -f '/opt/slemp/server/panel/tools.py' ];then
		...
		python /opt/slemp/server/panel/tools.pyc root $mysqlpwd
	fi
}
Close_MySQL()
{
	...
	if [ -f "${Setup_Path}/bin/mysql" ];then
		...
		rm -rf ${Setup_Path}
		mkdir -p /opt/slemp/backup
		[ -d "/opt/slemp/backup/oldData" ] && rm -rf /opt/slemp/backup/oldData
		mv -f $Data_Path  /opt/slemp/backup/oldData
		rm -rf $Data_Path
		rm -f /usr/bin/mysql*
		rm -f /usr/lib/libmysql*
		rm -f /usr/lib64/libmysql*
